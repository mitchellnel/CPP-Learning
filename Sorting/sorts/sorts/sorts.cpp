...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    for (int unsorted = 1; unsorted < s.size(); unsorted++) {
            // s[0] to s[unsorted-1] is sorted, we've push_backed to s[unsorted] == s[s.size()-1]
            // We need to find the correct position (loc) in the vector for s[unsorted]
        Student nextItem = s[unsorted];
        int loc = unsorted;             // where our unsorted item currently is
        
        while (loc > 0 && comp(nextItem, s[loc-1])) {
            s[loc] = s[loc-1];     // move the lower GPA Student to the right
            loc--;
        }
        
        // At this point, loc is where the recently added Student should belong
        s[loc] = nextItem;
    }
      ...
      // destruction time.
    {
     vector<Student> auxStudents(students);
      ...
      //        in students with the Students from auxStudents in the correct order.
        for (int i = 0; i < auxStudentsPtrs.size(); i++)
            students[i] = *auxStudentsPtrs[i];
